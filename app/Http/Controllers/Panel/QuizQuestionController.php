    ...
    public function store(Request $request)
{
    ...
    if (!empty($quiz) and $quiz->canAccessToEdit($user)) {
        ...
        // ✅ حول الـ quiz_ids إلى مصفوفة
        $webinarIds = is_array($quiz->quiz_ids) ? $quiz->quiz_ids : json_decode($quiz->quiz_ids, true);

        // ✅ تأكد إن المصفوفة مش فاضية
        if (empty($webinarIds)) {
            $webinarIds = [$quiz->id]; // fallback لو ما فيه ids
        }

        // ✅ لوب على كل webinar_id
        foreach ($webinarIds as $webinarId) {

            $order = QuizzesQuestion::query()->where('quiz_id', $webinarId)->count() + 1;
