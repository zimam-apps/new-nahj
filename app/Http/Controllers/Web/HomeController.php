...
class HomeController extends Controller
{
    public function index()
    {
        ...
        if (in_array(HomeSection::$diploma, $selectedSectionsName)) {
            $diplomaWebinars = Webinar::where('status', Webinar::$active)
                ->where('private', false)
                ->whereHas('category', function ($query) {
                    $query->where('id', '612')->orWhere('parent_id', '612');
                })
                ->orderBy('updated_at', 'desc')
                ->with([
                    'teacher' => function ($qu) {
        ...
        $data = [
            ...
            'diplomaWebinars' => $diplomaWebinars ?? [],
