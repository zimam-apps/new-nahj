...
class Bundle extends Model implements TranslatableContract
{
    ...
    {
        return $this->hasMany('App\Models\Studyschedule', 'bundle_id', 'id');
    }

    /**
     * Each Bundle Has Many Semesters "4 semesters"
     * Each Semester Have The Max hours that users is allowed to register.
     */
    public function semesters(): HasMany
    {
        return $this->hasMany(BundleSemester::class, 'bundle_id');
    }


    /**
     * Get the semester according to the given number
     */
    public function getSemester(int $semester_number)
    {
        $semester = BundleSemsterEnum::FIRST->value;
        if ($semester_number == 2) {
            $semester = BundleSemsterEnum::SECOND->value;
        } elseif ($semester_number == 3) {
            $semester = BundleSemsterEnum::THIRD->value;
        } elseif ($semester_number == 4) {
            $semester = BundleSemsterEnum::FOURTH->value;
        }
        return $this->semesters()->whereName($semester)->first();
    }
