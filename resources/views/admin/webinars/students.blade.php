    ...
    <div id="addStudentToCourseModal" class="d-none">
        ...
        <div class="mt-25">
            <form action="{{ getAdminPanelUrl() }}/enrollments/store" method="post">
                ...
                <div class="form-group">
                        <label class="input-label d-block font-weight-bold">أدخل المبلغ المدفوع إذا كنت ترغب في إنشاء فاتورة</label>
                        <input type="text" class="form-control" name="paid_amount" />
                        <div class="alert alert-light small mt-3">
    <div class="font-weight-bold">أترك حقل المبلغ المدفوع فارغاً إذا لم تكن تود إنشاء فاتورة</div>
    <div class="mt-1">معلومة: مبلغ الدورة التدريبية هو {{ $webinar->price }} ر.س</div>
</div>                    </div>
                    <div class="form-group">
                        <label class="input-label d-block">طريقة الدفع</label>
                        <select name="online_payment_method" required value="حوالة بنكية" class="form-control">
                            <option value="" disabled>أختر</option>
                            @foreach(['حوالة بنكية','شبكة','Tamara','Tabby','MADA','VISA','MASTERCARD'] as $onlinePaymentMethod)
                            <option value="{{ $onlinePaymentMethod }}">{{ $onlinePaymentMethod }}</option>             
                            @endforeach
                        </select>
                   </div>
                   <div class="form-group">
                        <label class="input-label d-block">رقم إيصال الدفع (اختياري)</label>
                        <input type="text" class="form-control" name="online_payment_method_id" />
                        <div class="small mt-2 text-muted">في حال وجود رقم إيصال يرجى اضافته</div>
...
@push('scripts_bottom')
    ...
    <script>
        ...
        function onChangeCreateInvoice(a) {
            if(a.selectedIndex == 0){
                console.log('no');
                console.log($(document).find('#invoice_form_control').index());
                $(document).find('#invoice_form_control').css('display','none');
            }else {
                console.log('yes');
                $(document).find('#invoice_form_control').css('display','block');
            }
        }
    </script>
