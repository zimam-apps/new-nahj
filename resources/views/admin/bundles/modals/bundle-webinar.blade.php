...
<div class="d-none" id="bundleWebinarsModal">
    ...
    <div class="js-form" data-action="{{ getAdminPanelUrl() }}/bundle-webinars/store">
        ...
        <div class="form-group mt-15">
            <label class="input-label d-block">{{ trans('panel.select_semester') }}</label>
            <select name="semester_id" id="semester_id" class="js-ajax-webinar_id form-control bundleWebinars-select" data-bundle-id="{{  !empty($bundle) ? $bundle->id : '' }}" data-placeholder="{{ trans('panel.select_course') }}">

                @if(!empty($bundle) and count($bundle->semesters))
                    @foreach($bundle->semesters as $semester)
                        <option value="{{ $semester->id }}">{{ __('public.semester_trans.'.$semester->name) }}</option>
                    @endforeach
                @endif
            </select>
            <div class="invalid-feedback"></div>
