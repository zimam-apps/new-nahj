                ...
                <div class="row">
                    ...
                    <div class="col-6 col-md-3 mt-30 mt-md-0 d-flex align-items-center justify-content-center mt-5 mt-md-0">
                        <div class="d-flex flex-column align-items-center text-center">
                            ...
                            @else
                            <strong class="font-10 font-weight-bold text-secondary mt-5">{{trans('quiz.unshow_result_to_student')}}</strong>
                            @endif
                        ...
                        <div class="rounded-lg shadow-sm py-25 px-20">
                            <div class="quiz-card">
                                <div class="d-flex align-items-center justify-content-between">
                                    <div>
                                        <h3 class="font-weight-bold font-16 text-secondary" dir="{{$quiz->text_direction ?? 'ltr' }}">{{ $question->title }}?</h3>
                                        <p class="text-gray font-14 mt-5">
                                ...
                                @if($question->type === \App\Models\QuizzesQuestion::$descriptive)
                                    ...
                                    <div class="form-group mt-35">
                                        ...
                                        <textarea name="question[{{ $question->id }}][answer]" rows="10" disabled class="form-control" dir="{{$quiz->text_direction ?? 'ltr' }}">{{ (!empty($userAnswers[$question->id]) and !empty($userAnswers[$question->id]["answer"])) ? $userAnswers[$question->id]["answer"] : '' }}</textarea>
                                    </div>

                                    <div class="form-group mt-35">
                                        <label class="input-label text-secondary" >{{ trans('quiz.correct_answer') }}</label>
                                        <textarea rows="10" name="question[{{ $question->id }}][correct_answer]" @if(empty($newQuizStart) or $newQuizStart->quiz->creator_id != $authUser->id) disabled @endif class="form-control" dir="{{$quiz->text_direction ?? 'ltr' }}">{{ $question->correct }}</textarea>
                                    </div>
                                    ...
                                    <div class="question-multi-answers mt-35">
                                        @foreach($question->quizzesQuestionsAnswers as $key => $answer)
                                            <div class="answer-item">
                                                @if($answer->correct)
                                                    <span class="badge badge-primary correct"dir="{{$quiz->text_direction ?? 'ltr' }}">{{ trans('quiz.correct') }}</span>
                                                @endif
                                                ...
                                                @if(!$answer->image)
                                                    <label for="asw-{{ $answer->id }}" class="answer-label font-16 d-flex text-dark-blue align-items-center justify-content-center ">
                                                        <span class="answer-title" dir="{{$quiz->text_direction ?? 'ltr' }}">
                                                            {{ $answer->title }}
