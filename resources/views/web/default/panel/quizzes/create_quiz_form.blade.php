    ...
    <div data-action="{{ !empty($quiz) ? '/panel/quizzes/' . $quiz->id . '/update' : '/panel/quizzes/store' }}"
        ...
        <section>
            ...
            <div class="row">
                <div class="col-12 col-md-4">
                    ...
                    @else
                        <input type="hidden" name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][locale]"
                            value="{{ $defaultLocale }}">
                    @endif
                    ...
                    @if (empty($selectedWebinar))
                        <div class="form-group mt-25">
                            ...
                            @if (empty($quiz) || empty($quiz->id))
                                {{-- ✅ الحالة الأولى: الاختبار جديد → نعرض Checkboxes --}}
                                <div class="d-flex flex-wrap">
                                    @foreach ($webinars as $webinar)
                                        <div class="custom-control custom-checkbox mr-20 mb-10">
                                            <input type="checkbox" class="custom-control-input"
                                                id="webinarCheck{{ $webinar->id }}" name="ajax[new][webinar_ids][]"
                                                value="{{ $webinar->id }}">
                                            <label class="custom-control-label" for="webinarCheck{{ $webinar->id }}">
                                                {{ $webinar->title }}
                                            </label>
                                        </div>
                                    @endforeach
                            ...
                            @else
                                {{-- ✅ الحالة الثانية: الاختبار موجود → نعرض قائمة dropdown --}}
                                <select name="ajax[{{ $quiz->id }}][webinar_id]"
                                    class="js-ajax-webinar_id custom-select">
                                    <option selected disabled value="">{{ trans('panel.choose_webinar') }}
                                    </option>
                                    @foreach ($webinars as $webinar)
                                        <option value="{{ $webinar->id }}"
                                            {{ $quiz->webinar_id == $webinar->id ? 'selected' : '' }}>
                                            {{ $webinar->title }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    @else
                        {{-- ✅ حالة وجود selectedWebinar --}}
                        <input type="hidden" name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][webinar_id]"
                    ...
                    @if (!empty($quiz))
                        <div class="form-group">
                            ...
                            <select name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][chapter_id]"
                    ...
                    @else
                        <input type="hidden" name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][chapter_id]"
                            value="" class="chapter-input">
                    @endif
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][time]"
                            value="{{ !empty($quiz) ? $quiz->time : old('time') }}" class="js-ajax-time form-control"
                            placeholder="{{ trans('forms.empty_means_unlimited') }}" min="0" />
                        <div class="invalid-feedback"></div>
                    </div>

                    <div class="form-group">
                        <label class="input-label">{{ trans('quiz.number_of_attemps') }}</label>
                        <input type="number" name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][attempt]"
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][pass_mark]"
                            value="{{ !empty($quiz) ? $quiz->pass_mark : old('pass_mark') }}"
                            class="js-ajax-pass_mark form-control " min="0" />
                        <div class="invalid-feedback"></div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][expiry_days]"
                            ...
                            class="js-ajax-expiry_days form-control" min="0" />
                        <div class="invalid-feedback"></div>


                        <div class="form-group mt-25">
                            <label class="input-label">{{ trans('quiz.text_direction') }}</label>

                            <select name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][text_direction]"
                                class="js-ajax-text_direction custom-select">
                                <option value="rtl"
                                    {{ !empty($quiz) && $quiz->text_direction == 'rtl' ? 'selected' : '' }}>
                                    عربي
                                </option>
                                <option value="ltr"
                                    {{ !empty($quiz) && $quiz->text_direction == 'ltr' ? 'selected' : '' }}>
                                    English
                                </option>
                    ...
                    @if (!empty($quiz))
                        <div class="form-group mt-20 d-flex align-items-center justify-content-between">
                            <label class="cursor-pointer input-label"
                                for="displayLimitedQuestionsSwitch{{ $quiz->id }}">{{ trans('update.display_limited_questions') }}</label>
                            <div class="custom-control custom-switch">
                                <input type="checkbox"
                                    name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][display_limited_questions]"
                                    class="js-ajax-display_limited_questions custom-control-input"
                                    id="displayLimitedQuestionsSwitch{{ $quiz->id }}"
                                    {{ $quiz->display_limited_questions ? 'checked' : '' }}>
                                <label class="custom-control-label"
                                    for="displayLimitedQuestionsSwitch{{ $quiz->id }}"></label>
                            </div>
                        </div>
                    ...
                    <div class="form-group mt-20 d-flex align-items-center justify-content-between">
                        <label class="cursor-pointer input-label"
                            for="certificateSwitch{{ !empty($quiz) ? $quiz->id : 'record' }}">{{ trans('quiz.certificate_included') }}</label>
                        <div class="custom-control custom-switch">
                            <input type="checkbox" name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][certificate]"
                                class="js-ajax-certificate custom-control-input"
                                id="certificateSwitch{{ !empty($quiz) ? $quiz->id : 'record' }}"
                                {{ !empty($quiz) && $quiz->certificate ? 'checked' : '' }}>
                            <label class="custom-control-label"
                                for="certificateSwitch{{ !empty($quiz) ? $quiz->id : 'record' }}"></label>
                        </div>
                    </div>
                    ...
                    <div class="form-group mt-20 d-flex align-items-center justify-content-between">
                        ...
                        <div class="custom-control custom-switch">
                            <input type="checkbox" name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][status]"
                    ...
                    <div class="form-group mt-20 d-flex align-items-center justify-content-between">
                        <label class="cursor-pointer input-label"
                            for="studentSwitch{{ !empty($quiz) ? $quiz->id : 'record' }}">
                            {{ trans('quiz.show_result_to_student') }}
                        </label>
                        <div class="custom-control custom-switch">
                            <input type="checkbox"
                                name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][show_result_to_student]"
                                class="js-ajax-show-result custom-control-input"
                                id="studentSwitch{{ !empty($quiz) ? $quiz->id : 'record' }}"
                                {{ !empty($quiz) && $quiz->show_result_to_student ? 'checked' : '' }}>
                            <label class="custom-control-label"
                                for="studentSwitch{{ !empty($quiz) ? $quiz->id : 'record' }}"></label>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        @foreach ($quizQuestions as $question)
                            <li data-id="{{ $question->id }}"
                                ...
                                <div class="flex-grow-1">
                                    <h4 class="question-title" dir="{{ $quiz->text_direction ?? 'ltr' }}">
                                        {{ $question->title }}</h4>
        ...
        <div class="mt-20 mb-20">
            ...
            @if (empty($quiz) and !empty($inWebinarPage))
                <button type="button"
                    class="btn btn-sm btn-danger mr-10 cancel-accordion">{{ trans('public.close') }}</button>
            @endif
        </div>
    </div>
