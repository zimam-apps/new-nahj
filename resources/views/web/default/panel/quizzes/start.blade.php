...
@section('content')
    <div class="container">
        <section class="mt-40">
            <h2 class="font-weight-bold font-16 text-dark-blue" dir="{{$question->text_direction ?? 'ltr' }}">{{ $quiz->title }}</h2>
                    ...
                    <fieldset class="question-step question-step-{{ $key + 1 }}">
                        <div class="rounded-lg shadow-sm py-25 px-20">
                            <div class="quiz-card">
                                ...
                                <div class="">
                                    <h3 class="font-weight-bold font-16 text-secondary" dir="{{$question->text_direction ?? 'ltr' }}">{{ $question->title }}</h3>
                                </div>

                                @if($question->type === \App\Models\QuizzesQuestion::$descriptive)
                                    <div class="form-group mt-35">
                                        <textarea name="question[{{ $question->id }}][answer]" rows="15" class="form-control" dir="{{$question->text_direction ?? 'ltr' }}"></textarea>
                                    </div>
                                @else
                                            ...
                                            <div class="answer-item">
                                                ...
                                                @if(!$answer->image)
                                                    <label for="asw-{{ $answer->id }}" class="answer-label font-16 text-dark-blue d-flex align-items-center justify-content-center ">
                                                            <span class="answer-title" dir="{{$question->text_direction ?? 'ltr' }}">
                                                                {{ $answer->title }}
                                                            </span>
                                                    </label>
                                                @else
